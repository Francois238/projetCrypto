...
pub fn revocation_ext(mail: String, otp: String, motif_revocation: String) -> Result<(), ApiError> {
    let tab_cert_file = fs::read_to_string("liste_certificats.json")
        .map_err(|_| ApiError::new(404, "Votre certificat n'existe pas".to_string()))?;
    ...
    for cert in tab_cert.iter() {
        if cert.mail == mail && cert.otp == otp {
            //on a trouvé le certificat à révoquer
    ...
    if found {
        ...
        fs::write("liste_certificats.json", tab_cert)
            .map_err(|_| ApiError::new(500, "erreur interne".to_string()))?;

        Ok(())
    } else {
        Err(ApiError::new(
            404,
            "Votre certificat n'existe pas".to_string(),
        ))
    }
}
...
fn store_motif(motif: String) -> Result<(), ApiError> {
    ...
    if tab_motif_file.is_ok() {
        //le fichier existe

        tab_motif = serde_json::from_str(&tab_motif_file.map_err(|_| {
            ApiError::new(
                500,
                "Impossible de lire fichier motif revocation".to_string(),
            )
        })?)
        .map_err(|_| {
            ApiError::new(
    ...
    let tab_motif = serde_json::to_string(&tab_motif).map_err(|_| {
        ApiError::new(
            500,
            "Impossible d'enregistrer fichier motif revocation".to_string(),
        )
    })?;

    fs::write("liste_motifs.json", tab_motif).map_err(|_| {
        ApiError::new(
            500,
            "Impossible d'enregistrer fichier motif revocation".to_string(),
        )
...
fn revocation(file: String) -> Result<(), ApiError> {
    ...
    let id = fs::read_to_string("ocsp_process_id")
        .map_err(|_| ApiError::new(500, "Impossible de lire le process id".to_string()))?;
...
pub fn run_ocsp_server() -> Result<(), ApiError> {
    ...
    if !file.exists() {
        ...
        let _index = File::create("demoCA/index.txt").map_err(|_| {
            ApiError::new(500, "Impossible de créer le fichier index.txt".to_string())
        })?;
    }
    ...
    fs::write("ocsp_process_id", id.to_string())
        .map_err(|_| ApiError::new(500, "Impossible d'enregistrer le process id".to_string()))?;
